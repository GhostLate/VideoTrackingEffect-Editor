...
class AudioOutput(QAudioOutput):
    ...
    def __init__(self, player_window: UIMainWindow):
        ...
        def onSliderPositionChange():
            ...
            if self.player_window.control_panel.volumeSlider.value() == 0 \
                    and not self.player_window.control_panel.volumeButton.is_clicked:
                self.player_window.control_panel.volumeButton.switch_state()
            if self.player_window.control_panel.volumeSlider.value() > 0 \
                    and self.player_window.control_panel.volumeButton.is_clicked:
                self.player_window.control_panel.volumeButton.switch_state()
